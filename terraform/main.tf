...
resource "aws_kms_key" "data" {
  description             = "CGE-P capstone data protection key"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Principal = { Service = "cloudtrail.amazonaws.com" }
        Action    = "s3:PutObject"
        Resource  = "${aws_s3_bucket.evidence_vault.arn}/AWSLogs/${data.aws_caller_identity.current.account_id}/*"
        Condition = {
          StringEquals = {
...
resource "aws_cloudtrail" "capstone" {
  ...
  depends_on = [aws_s3_bucket_policy.evidence_vault]
}
...
resource "aws_security_group" "logs_endpoint" {
  name        = "${local.name_prefix}-logs-endpoint-sg-${local.suffix}"
  description = "Allow Lambda HTTPS to CloudWatch Logs endpoint"
  vpc_id      = aws_vpc.main.id
  ...
  tags = {
    Name = "${local.name_prefix}-logs-endpoint-sg"
  }
}
...
resource "aws_security_group" "lambda" {
  ...
  description = "Egress for the private intake Lambda"
  vpc_id      = aws_vpc.main.id

  egress {
    description = "Allow HTTPS to approved AWS service endpoints"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
...
######################################################################
# API Gateway — regional REST API in front of the Lambda.
# REST API is required because AWS WAF associates with REST API stages.
######################################################################
...
resource "aws_api_gateway_deployment" "intake" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_method.intake.id,
...
resource "aws_api_gateway_stage" "default" {
  ...
  depends_on = [aws_api_gateway_account.regional]
...
resource "aws_lambda_permission" "apigw" {
  ...
  source_arn    = "${aws_api_gateway_rest_api.intake.execution_arn}/*/POST/intake"
}

resource "aws_iam_role" "apigw_cloudwatch" {
  name = "${local.name_prefix}-apigw-cloudwatch-${local.suffix}"
